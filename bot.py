...
from resilience import sync_panel_configs
...
async def run_bot():
    ...
    asyncio.create_task(panel_sync_loop())
...
PANEL_SYNC_SECONDS = 600

async def panel_sync_loop():
    """Every 10 minutes refresh VIP panel-backed services from the authoritative panel."""
    while True:
        try:
            result = await sync_panel_configs()
            if result.get("updated") or result.get("failed"):
                logger.info("panel sync: %s", result)
        except Exception:
            logger.exception("خطا در همگام‌سازی دوره‌ای پنل‌ها")
        await asyncio.sleep(PANEL_SYNC_SECONDS)
